...
# Adding service credentials to your Kubernetes environment
...
1. Install the [{{site.data.keyword.dev_cli_long}} CLI](/docs/cli?topic=cloud-cli-getting-started).
...
3. Click **Download code** to regenerate your app with the injected code snippets.
...
4. To access the credentials locally, copy and replace the following files from the newly generated `.zip` file, to your local Git clone to access the credentials. You must still create a Kubernetes secret in your cluster to host the credentials.
   ...
   - `src/main/resources/mappings.json` - A mapping to provide access to the [`env.getProperty()`](/docs/java?topic=java-spring-configuration) method to access your environment variables from the code.
...
1. To view the results, run this command:
