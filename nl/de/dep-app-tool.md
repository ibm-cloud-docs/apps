...
Zur Vorbereitung müssen Sie mit dem [{{site.data.keyword.cloud_notm}}-Dashboard](https://{DomainName}){: new_window} ![Symbol für externen Link](../icons/launch-glyph.svg "Symbol für externen Link") Ihre [App erstellen](/docs/apps?topic=creating-apps-getting-started) und [Services hinzufügen](/docs/apps?topic=creating-apps-getting-started#resources-getting-started).
...
Continuous Delivery ist für manche automatisch Apps aktiviert. Sie können Continuous Delivery aktivieren, um Builds, Tests und Bereitstellungen über die Delivery Pipeline und GitHub zu automatisieren.
...
Bevor Sie beginnen, müssen Sie die {{site.data.keyword.cloud_notm}}-CLI [herunterladen und installieren](/docs/cli?topic=cloud-cli-getting-started).
...
{: important}
  ...
  3.  Aktualisieren Sie Ihren App-Code. Beispiel: Wenn Sie eine {{site.data.keyword.cloud_notm}}-Beispiel-App verwenden und Ihre App die Datei `src/main/webapp/index.html` enthält, können Sie diese ändern und den `Dankestext` bearbeiten. Prüfen Sie, ob sich die App lokal ausführen lässt, bevor Sie sie in {{site.data.keyword.cloud_notm}} bereitstellen.

    Prüfen Sie den Inhalt der `README.md`-Datei, die Details wie z. B. Erstellungsanweisungen enthält.

    Wenn es sich bei Ihrer App um eine Liberty-App handelt, müssen Sie sie erstellen, bevor Sie sie erneut bereitstellen.
    {: note}
...
* [Deploying to {{site.data.keyword.cloud_notm}} Environments with {{site.data.keyword.dev_cli_short}} CLI](https://www.ibm.com/cloud/blog/deploying-to-ibm-cloud-environments-with-ibm-cloud-developer-tools-cli){: new_window} ![Symbol für externen Link](../icons/launch-glyph.svg "Symbol für externen Link")
