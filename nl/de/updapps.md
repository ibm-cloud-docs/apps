...
### Verwendung der {{site.data.keyword.Bluemix_notm}}-Benutzerschnittstelle:
  ...
  1. Erstellen Sie eine angepasste Domäne für Ihre Organisation.
	...
	1. Wechseln Sie zu **Verwalten** &gt; **Konto** &gt; **Cloud Foundry-Organisationen** &gt; **Details anzeigen** für Ihre Organisation. Klicken Sie anschließend auf **Organisation bearbeiten** &gt; **Domänen**.
...
### Verwendung der Befehlszeilenschnittstelle 'bluemix':
  ...
  1. Erstellen Sie eine angepasste Domäne für Ihre Organisation, indem Sie den folgenden Befehl eingeben:
    ...
    *Organisationsname*: Der Name Ihrer Organisation.

    *mydomain*: Der Name, den Sie für die angepasste Domäne verwenden wollen.
  ...
  2. Fügen Sie die Route mit der angepassten Domäne zu einer Anwendung hinzu. Geben Sie für CF-Apps den folgenden Befehl ein:
    ...
    *myapp*: Bei CF-Apps der Name der Anwendung.

    *mydomain*: Der Name Ihrer angepassten Domäne, z. B. `www.mydomain.mybluemix.net`.

    *host_name*: Der Hostname in der Route, den Sie für Ihre Anwendung verwenden wollen.

    *mycontainergroup*: Bei Containergruppen der Name der Containergruppe.
...
###Beispiel: Befehl 'bluemix app rename' verwenden

In diesem Beispiel lautet der Name der Anwendung 'Blue'. Das Beispiel veranschaulicht, wie die Version von *Blue* mithilfe des Befehls **bluemix app rename** aktualisiert wird, ohne den Datenverkehr zu der Anwendung zu unterbrechen. Optional kann die dann alte Version von *Blue* gelöscht werden, wenn die aktualisierte Version vorhanden ist.
...
2. Verwenden Sie den Befehl **bluemix app rename**, um die App *Blue* in *Green* zu ändern:

  ```
  bluemix app rename Blue Green
  ```

  Listen Sie die Anwendungen im aktuellen Bereich mit dem Befehl **bluemix app list** auf:
...
4. Optional: Wenn Sie die alte Version (*Green*) der App löschen wollen, verwenden Sie den Befehl **bluemix app delete**.

  ```
  bluemix app delete Green -f
  ```

  Listen Sie die Routen in Ihrem Bereich mit dem Befehl **bluemix app route-map** auf:
...
###Beispiel: Befehl 'bluemix app route-map' verwenden

In diesem Beispiel ist *Blue* die zuvor bereitgestellte Anwendung, und *Green* ist die aktualisierte Version. Dieses Beispiel veranschaulicht, wie die Version von *Blue* mithilfe des Befehls **bluemix app route-map** aktualisiert wird, ohne den Datenverkehr zu der Anwendung zu unterbrechen. Optional kann die dann alte Version von *Blue* gelöscht werden, wenn die aktualisierte Version vorhanden ist.
...
2. Nehmen Sie die erforderlichen Änderungen vor und bereiten Sie die Version *Green* vor. Stellen Sie die App *Green* per Push-Operation in {{site.data.keyword.Bluemix_notm}} bereit:
  ...
  Listen Sie die Anwendungen im aktuellen Bereich mit dem Befehl **bluemix app route-map** auf:
...
6. Optional: Wenn Sie die alte Version (*Blue*) der Anwendung löschen wollen, verwenden Sie hierzu den Befehl `bluemix app delete`.
