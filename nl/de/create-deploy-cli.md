...
Sie können Ihre App über die Befehlszeilenschnittstelle von {{site.data.keyword.cloud}} erstellen und bereitstellen. 
...
Diese Option kann verwendet werden, wenn Sie bereits über eine vorhandene Codebasis verfügen und Assets für die Bereitstellung und die Cloudaktivierung für einen einzelnen Mikroservice oder eine einzelne Web-App generieren möchten, indem Sie [`ibmcloud dev enable`](/docs/cli/idt/commands.html#enable) verwenden. Beachten Sie, dass dieser Befehl in der Betaversion enthalten ist und nicht alle Sprachen und/oder App-Strukturen unterstützt werden. Die folgenden Anweisungen veranschaulichen, wie diese Funktionalität bei einem Beispielrepository verwendet wird. Die Schritte sind jedoch bei Ihrer eigenen Codebasis in etwa dieselben.

1. Melden Sie sich bei IBM Cloud an, indem Sie 'ibmcloud login' ausführen, und wählen Sie anschließend eine Organisation und einen Bereich als Ziel aus.
2. Klonen Sie die [Hello World-Beispiel-App](https://github.com/IBM-Cloud/node-helloworld), indem Sie den folgenden Befehl in dem von Ihnen ausgewählten Verzeichnis ausführen.
...
3. Navigieren Sie zu dem Verzeichnis, in dem Sie die Beispiel-App geklont haben, und führen Sie den Befehl [`ibmcloud dev enable`](/docs/cli/idt/commands.html#enable) aus.
...
7. Wählen Sie die Option zum Erstellen einer neuen {{site.data.keyword.Bluemix_notm}}-App, die mit diesem Git-Repository verknüpft ist, aus. Details hierzu finden Sie unter **Wichtige Hinweise**.
...
10. Wenn sie abgeschlossen sind, können Sie die Bereitstellungs- und Cloudaktivierungsdateien, die im App-Verzeichnis gespeichert werden, manuell zusammenführen. Alle neuen Dateien, die mit `.merge` markiert sind, sollten mit `git diff` oder einem ähnlichen Tool zusammengeführt werden.
...
### Wichtige Hinweise
 ...
 - Sie können auch nur Bereitstellungs- und Cloudaktivierungsdateien generieren, ohne dass eine Verbindung zu einer {{site.data.keyword.cloud_notm}}-App hergestellt wird, indem Sie [`ibmcloud dev enable --no-create`](/docs/cli/idt/commands.html#enable) ausführen.
 - Wenn Sie eine Toolchain und Bereitstellungsdateien manuell konfigurieren möchten, folgen Sie  [dieser Anleitung.](/docs/apps/tutorials/tutorial_byoc_kube.html#tutorial) Dies kann sinnvoll sein, wenn Sie versuchen, eine Continuous-Delivery-Toolchain für mehrere in Wechselbeziehung zueinander stehende Web-Apps oder Mikroservices zu konfigurieren.
...
4. Sie können die lokal ausgeführte App über `http://localhost:3000` oder eine ähnliche URL anzeigen.
...
Abhängig von der Konfiguration Ihrer App stehen zwei Möglichkeiten für die Bereitstellung Ihrer {{site.data.keyword.Bluemix_notm}}-App zur Verfügung. 

### App mithilfe einer DevOps-Toolchain bereitstellen
Wenn Sie noch keine DevOps-Toolchain für Ihre App erstellt haben und Ihre App sich noch nicht in eimem Git-Repository befindet, können Sie den Befehl [`ibmcloud dev edit`](/docs/cli/idt/commands.html#edit) ausführen. Folgen Sie den Eingabeaufforderungen für die DevOps-Konfiguration und führen Sie eine Bereitstellung für eine neue Toolchain durch (und erstellen Sie ein neues GitLab-Repository).

Wenn Sie eine DevOps-Toolchain für Ihre App erstellt haben, müssen Sie zur Bereitstellung eines neuen Builds lediglich Ihren Code festschreiben und per Push-Operation in das Repository in der Toolchain übertragen. 
...
4. Zeigen Sie die DevOps-Toolchain für Ihre App über die {{site.data.keyword.Bluemix_notm}}-Konsole an. Sie können Toolchain-Details aus der Anzeige mit den App-Details in der {{site.data.keyword.Bluemix_notm}}-Konsole anzeigen, indem Sie den Befehl [`ibmcloud dev console`](/docs/cli/idt/commands.html#console) im App-Verzeichnis ausführen.
...
1. Stellen Sie sicher, dass [ein Kubernetes-Cluster erstellt wurde](https://{DomainName}/containers-kubernetes/overview).
...
**Wenn Sie Probleme melden oder Feedback geben möchten, können Sie den [Slack für IBM Cloud Tech - Kanal für Entwicklertools (#developer-tools)](https://ibm-cloud-tech.slack.com) aufrufen - Fordern Sie [hier](https://slack-invite-ibm-cloud-tech.mybluemix.net/) Zugriff für Ihr Team an.**
