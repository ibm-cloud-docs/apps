...
### 使用 {{site.data.keyword.Bluemix_notm}} 主控台
  ...
  1. 建立組織的自訂網域。
	...
	1. 針對您的組織，移至**管理** &gt; **帳戶** &gt; **Cloud Foundry 組織** &gt; **檢視詳細資料**。然後，按一下**編輯 Cloud Foundry 組織** &gt; **網域**。
...
### 使用 Bluemix 指令行介面
  ...
  1. 鍵入下列指令以建立組織的自訂網域：
    ...
    *organization_name*：組織的名稱。

    *mydomain*：您要使用的自訂網域名稱。
  ...
  2. 新增含有自訂網域的路徑至應用程式。若為 CF 應用程式，請鍵入下列指令：
    ...
    *myapp*：若為 CF 應用程式，則是應用程式的名稱。

    *mydomain*：您的自訂網域名稱，例如 `www.mydomain.mybluemix.net`。

    *host_name*：您要用於應用程式之路徑中的主機名稱。

    *mycontainergroup*：若為容器群組，則是容器群組的名稱。
...
###範例：使用 bluemix app rename 指令

在此範例中，應用程式的名稱為 Blue。此範例示範如何使用 **bluemix app rename** 指令來更新 *Blue* 的版本，而不用中斷應用程式的資料流量。更新版本安裝完成時，即可選擇性地刪除舊版的 *Blue*。
...
2. 使用 **bluemix app rename** 指令，將 *Blue* 應用程式重新命名為 *Green*：

  ```
  bluemix app rename Blue Green
  ```

  使用 **bluemix app list** 指令，列出現行空間中的應用程式：
...
4. 選用項目：如果您要刪除舊版 (*Green*) 的應用程式，請使用 **bluemix app delete** 指令。

  ```
  bluemix app delete Green -f
  ```

  使用 **bluemix app route-map** 指令，列出空間中的路徑：
...
###範例：使用 bluemix app route-map 指令

在此範例中，*Blue* 是先前部署的應用程式，而 *Green* 是更新後的版本。此範例示範如何使用 **bluemix app route-map** 指令更新 *Blue* 的版本，而不用中斷應用程式的資料流量。更新版本安裝完成時，即可選擇性地刪除舊版的 *Blue*。
...
2. 進行必要變更，並備妥 *Green* 版本。將 *Green* 應用程式推送至 {{site.data.keyword.Bluemix_notm}}：
  ...
  使用 **bluemix app route-map** 指令，列出現行空間中的應用程式：
...
6. 選用項目：如果您要刪除舊版 (*Blue*) 的應用程式，請使用 `bluemix app delete` 指令。
