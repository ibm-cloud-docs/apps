...
您可以從基本 Web 入門範本建立應用程式。請使用這些入門範本，來設定具有所選擇 Web 架構之 Swift、Node、Java 或 Python 的簡單 Web 伺服器。您可以查看如何安裝所需的工具、在本端建置並執行應用程式，然後將它部署至雲端。
{: shortdesc}
...
1. 從 {{site.data.keyword.dev_console}} 的[入門範本套件 ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://console.ng.bluemix.net/developer/appservice/starter-kits/) 頁面中，選取您語言的入門範本套件。例如，若為 Node.js 應用程式，請選取 **Express.js 基本**及**建立**。

2. 輸入應用程式名稱。針對本指導教學，請使用 `WebBasicProject`。   
...
5. 使用瀏覽器開啟 `http://localhost:3000`。您的埠號可能會根據選擇的運行環境而不同。
