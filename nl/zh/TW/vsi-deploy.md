...
若要檢視這些環境內容，請遵循下列步驟。請參閱表格以瞭解可用的內容。
...
Terraform 需要基礎架構 API 金鑰以便建立基礎架構資源。這是在部署期間自動取得。請遵循下列步驟來手動擷取金鑰。
...
基礎架構使用者名稱也會在部署期間自動取得及使用。請遵循下列步驟來手動擷取金鑰。
...
為了在 Terraform 中建立像是資料庫的平台層次服務，以及組合服務，平台 API 金鑰會自動取得並儲存為您管線中的環境變數。請遵循下列步驟來手動擷取平台金鑰。
