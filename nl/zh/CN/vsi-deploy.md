...
与其他配置相比，虚拟服务器实例可以为所有工作负载类型提供更好的透明度、可预测性和自动化功能。与裸机服务器相结合，还可以创建出独特的工作负载组合。例如，可以运行基于 Debian Linux 的操作系统，使用裸机和 GPU 配置创建高性能的数据库逻辑或机器学习。
...
## 通过 Terraform 部署
在动态创建的虚拟实例中，可以通过 [Terraform ![外部链接图标](../icons/launch-glyph.svg)](https://ibm-cloud.github.io/tf-ibm-docs/v0.10.0/){: new_window} 部署任何 App Service 初学者工具包。Terraform 是一种开放式源代码“基础架构即代码”框架。有关更多信息，请参阅 [Terraform 文档 ![外部链接图标](../icons/launch-glyph.svg)](https://www.terraform.io/docs/index.html){: new_window}。

## 启用管道部署

当您创建初学者工具包来使用 {{site.data.keyword.cloud_notm}} [App Service ![外部链接图标](../icons/launch-glyph.svg)](https://console.bluemix.net/developer/appservice/starter-kits){: new_window} 时，系统会启用虚拟服务器实例。在创建应用程序后，可以选择要在何处部署应用程序。使用 Continous Delivery 工具链，可以启用初学者工具包来支持部署。初学者工具包可用于 Kubernetes、Cloud Foundry 以及 Virtual Server 实例。该工具链包含源代码存储库和部署管道。
...
要查看环境属性，并配置用户帐户属性来启用管道，请执行以下步骤。

1. 从“应用程序详细信息”页面，单击**查看工具链**。
2. 单击 **Delivery Pipeline** 磁贴。
3. 在构建阶段上，单击**配置阶段**。
4. 单击**环境变量**选项卡来查看属性。
5. 在“环境变量”选项卡中更改这些属性，以使工具链能够成功运行。
...
| `PRIVATE_KEY` | [专用密钥](#public-key)，可定义该密钥来允许对虚拟服务器实例的访问。|
...
1. 转至基础架构[用户列表 ![外部链接图标](../icons/launch-glyph.svg)](https://control.bluemix.net/account/users){: new_window} 或**菜单** > **基础架构** > **帐户** > **用户列表**。
2. 查找创建工具链的用户的详细信息，单击“API 密钥”列中的`查看`或单击`生成`，这两个步骤都会在对话框中显示 API 密钥。
...
要使 Terraform 能够创建平台级别的服务（如数据库或 Compose 服务），需要平台 API 密钥。要检索平台密钥，请执行以下步骤。

1. 单击**菜单** > **仪表板**，以确保用户在平台仪表板中。
2. 单击**管理** > **安全性** > **平台 API 密钥**。
3. 单击**创建**。
4. 输入名称和描述，然后单击**创建**。
5. 在打开的对话框中，单击*显示*以查看密钥。
...
要使工具链能够将 Debian 软件包安装到虚拟服务器实例上，请执行以下步骤。

1. 在客户端计算机中，使用下面的指示信息来创建[公用和专用密钥对 ![外部链接图标](../icons/launch-glyph.svg)](https://help.github.com/articles/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent/){: new_window}。
2. 转至[基础架构 SSH 密钥视图 ![外部链接图标](../icons/launch-glyph.svg)](https://control.bluemix.net/devices/sshkeys){: new_window} 或**菜单** > **基础架构** > **设备** > **管理** > **SSH 密钥**。
3. 单击**添加**。
4. 将先前创建的专用密钥的内容复制并粘贴到密钥内容中。
5. 为密钥指定名称，然后单击**添加**。

### 启用 Terraform 状态
{: #tform-state}

Terraform 支持存储 Terraform `apply` 命令的状态。此状态文件会第二次运行 `apply` 命令，以确定是否有任何 terraform 配置文件发生更改。您可以在应用程序和配置管理的 GIT 存储库中存储状态。

要启用状态存储，请在工具链环境变量中配置 `GIT_USER` 和 `GIT_PASSWORD` 属性。要检索值，请执行以下步骤：

1. 在“工具链”视图中，访问“代码工具”中的 GIT 存储库。
2. 在 GIT Lab 中，单击**概要文件图标**，然后单击**设置**。
3. 单击**帐户**，复制用户名并替换 `GIT_USER` 值。
4. 单击**访问令牌**。
5. 定义令牌名称，然后单击**创建个人访问令牌**。
...
2. “Terraform 规划阶段”会准备一个 terraform 计划。
...
4. “部署、安装、启动阶段”会将第一阶段构建的 Debian 软件包移动到运行中虚拟服务器上，然后安装并启动该软件包。
5. “运行状况检查阶段”会验证应用程序上的运行状况端点是否可用，然后完善管道。
