...
### 使用 {{site.data.keyword.Bluemix_notm}} 控制台
  ...
  1. 为组织创建定制域。
	...
	1. 转至**管理** &gt; **帐户** &gt; **Cloud Foundry 组织** &gt; **查看详细信息**（针对您的组织）。然后，单击**编辑 Cloud Foundry 组织** &gt; **域**。
...
### 使用 bluemix 命令行界面
  ...
  2. 将包含定制域的路径添加到应用程序。对于 CF 应用程序，请输入以下命令：
    ...
    *myapp*：对于 CF 应用程序，为应用程序的名称。

    *mydomain*：定制域的名称，例如 `www.mydomain.mybluemix.net`。

    *host_name*：要用于应用程序的路径中的主机名。

    *mycontainergroup*：对于容器组，为容器组的名称。
...
###示例：使用 bluemix app rename 命令

在此示例中，应用程序的名称为 Blue。此示例演示了如何使用 **bluemix app rename** 命令来更新 *Blue* 的版本，而不中断进入应用程序的流量。现在，在更新版本可用时，可以选择删除 *Blue* 的旧版本。
...
2. 使用 **bluemix app rename** 命令将 *Blue* 应用程序重命名为 *Green*：

  ```
  bluemix app rename Blue Green
  ```

  使用 **bluemix app list** 命令列出当前空间中的应用程序：
...
4. 可选：如果想要删除应用程序的旧版本 (*Green*)，请使用 **bluemix app delete** 命令。

  ```
  bluemix app delete Green -f
  ```

  使用 **bluemix app route-map** 命令列出空间中的路径：
...
###示例：使用 bluemix app route-map 命令

在此示例中，*Blue* 是先前部署的应用程序，*Green* 是更新的版本。此示例演示了如何使用 **bluemix app route-map** 命令来更新 *Blue* 的版本，而不中断进入应用程序的流量。现在，在更新版本可用时，可以选择删除 *Blue* 的旧版本。
...
2. 进行必要的更改，并使 *Green* 版本准备就绪。将 *Green* 应用程序推送到 {{site.data.keyword.Bluemix_notm}}：
  ...
  使用 **bluemix app route-map** 命令列出当前空间中的应用程序：
...
6. 可选：如果想要删除应用程序的旧版本 (*Blue*)，请使用 `bluemix app delete` 命令。
  ...
  结果：*Green* 应用程序正在响应 URL `Blue.mybluemix.net`。
