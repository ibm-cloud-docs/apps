...
2. 在 {{site.data.keyword.dev_console}} 中的[入门模板工具包 ![外部链接图标](../../icons/launch-glyph.svg "外部链接图标")](https://{DomainName}/developer/appservice/starter-kits/) 页面中，根据您需要的功能来选择入门模板工具包。例如，对于 Watson Language 应用程序，选择 **Swift Kitura**。
3. 输入应用程序名称。对于本教程，请使用 `WatsonApp`。
4. 可选。提供标记来对应用程序分类。有关更多信息，请参阅[使用标记](/docs/resources/tagging_resources.html)。
