...
Puoi creare un'applicazione personalizzata da zero utilizzando i servizi e un runtime. Puoi vedere come installare gli strumenti di cui hai bisogno, creare ed eseguire l'applicazione localmente e distribuirla al cloud.
{: shortdesc}

## Passo 1. Installa gli strumenti
{: #install-tools}

Installa [{{site.data.keyword.dev_cli_long}}](/docs/cli/index.html).

Docker viene installato come parte di Developer Tools. Affinché i comandi di build funzionino è necessario che Docker sia in esecuzione. Devi creare un account Docker, eseguire l'applicazione Docker ed effettuare l'accesso.

## Passo 2. Crea un'applicazione da zero
{: #create-devex}

Creare un'applicazione personalizzata nel dashboard {{site.data.keyword.cloud}}:

1. Dal [dashboard {{site.data.keyword.cloud}} ![Icona link esterno](../../icons/launch-glyph.svg "Icona link esterno")](https://{DomainName}), seleziona **Crea** per creare un'applicazione personalizzata.

  Puoi anche creare un'applicazione personalizzata dalla pagina [Starter Kits ![Icona link esterno](../../icons/launch-glyph.svg "Icona link esterno")](https://{DomainName}/developer/appservice/starter-kits/) nella {{site.data.keyword.dev_console}}, dove selezioni **Crea** per creare un'applicazione personalizzata.
  {: tip}
...
## Passo 4. Distribuisci con una toolchain DevOps
{: #add-toolchain}
...
2. Seleziona un metodo di distribuzione. Configura il tuo metodo di distribuzione in base alle istruzioni per il metodo che scegli:
  ...
  * Distribuisci a una VSI (Virtual Server Instance).

La distribuzione della tua applicazione sul cloud nell'ultimo passo crea una toolchain automaticamente. La toolchain crea un repository Git per la tua applicazione dove puoi trovare il codice. Per ulteriori informazioni, vedi [Creazione delle toolchain](/docs/services/ContinuousDelivery/toolchains_working.html).

## Passo 5. Crea ed esegui l'applicazione localmente
{: #build-run}

Puoi anche creare localmente l'applicazione per l'esecuzione di test prima di eseguirne il push al cloud.
...
### Distribuisci con una toolchain DevOps
