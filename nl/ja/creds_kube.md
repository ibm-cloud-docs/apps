...
このケースでは、アプリケーションの作成元はスターター・キットです。スターター・キットで生成されたコードは、ローカル、Cloud Foundry、または Kubernetes で実行できるように移植可能になります。アプリケーション・コードと、リソース (サービス・インスタンス) の資格情報を保持する環境変数の取得との間の抽象化層を提供するために、ライブラリー `IBMCloudEnv` が使用されます。
