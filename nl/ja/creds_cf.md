...
Cloud Foundry デプロイメント環境にサービス資格情報を追加する方法について説明します。
{: shortdesc}
...
Cloud Foundry ベースのリソース・インスタンスが、デプロイされる Cloud Foundry アプリケーションと同じ Cloud Foundry スペース内にある場合は、[次のセクション](#cf_resource_same)を参照してください。

Cloud Foundry ベースのリソース・インスタンスが、Cloud Foundry アプリケーションのターゲット・スペースとは異なるスペースにある場合は、[その後のセクション](#cf_resource_different)を参照してください。

アプリケーションに関連付けたリソースがリソース・コントローラー・ベースの場合は、[リソース・コントローラー](#cf_resource_controller)を参照してください。
...
続行する前に、[スターター・キット・アプリ + Kubernetes](/docs/apps/creds_kube.html#sk_kube_generated_code) を参照してください。次に、以下の変更を適用します。
