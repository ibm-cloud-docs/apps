...
{: tsResolve}
  ...
  * Pour supprimer une instance de service, vous pouvez utiliser la console {{site.data.keyword.Bluemix_notm}} ou l'interface de ligne de commande.
    ...
    Pour utiliser l'interface de ligne de commande pour supprimer une instance de service, procédez comme suit :
	  1. Annulez la liaison de l'instance de service à l'application en entrant `cf unbind-service <appname> <service_instance_name>`.
	  2. Supprimez l'instance de service en entrant `cf delete-service <service_instance_name>`.
	  3. Une fois l'instance de service supprimée, vous pouvez reconstituer l'application à laquelle l'instance de service était liée en entrant `cf restage <appname>`.
