...
6. Sélectionnez l'option **IBM DevOps, en utilisant des packs de construction Cloud Foundry** pour créer une chaîne d'outils DevOps. Vous devrez peut-être configurer des clés SSH pour finaliser cette étape.
  ...
  {: note}
7. Entrez un nom d'hôte unique, par exemple, `abc-devhost`. Ce nom d'hôte correspond à la route de votre application, `abc-devhost.cloud.ibm.com`.
...
10. Une fois les opérations terminées, vous pouvez fusionner les fichiers de déploiement et d'activation de cloud sauvegardés dans le répertoire de l'application. Fusionnez les nouveaux fichiers marqués `.merge` en utilisant `git diff` ou un outil similaire.
...
### Remarques importantes
 ...
 - Vous pouvez également choisir de générer des fichiers de déploiement et d'activation de cloud sans les connecter à une application {{site.data.keyword.cloud_notm}} en exécutant [`ibmcloud dev enable --no-create`](/docs/cli/idt/commands.html#enable).
 - Pour configurer manuellement une chaîne d'outils et des fichiers de déploiement, suivez [le tutoriel](/docs/apps/tutorials/tutorial_byoc_kube.html#tutorial-byoc-kube). Cela peut être utile si vous tentez de configurer une chaîne d'outils Continuous Delivery pour plusieurs microservices ou applications Web interconnectés.
...
1. Pour afficher l'URL de votre application qui s'exécute sur {{site.data.keyword.cloud_notm}}, exécutez la commande [`ibmcloud dev view`](/docs/cli/idt/commands.html#view).
2. Pour afficher les détails relatifs aux données d'identification, aux services et à la chaîne d'outils de votre application à partir de la console {{site.data.keyword.cloud_notm}}, exécutez la commande [`ibmcloud dev console`](/docs/cli/idt/commands.html#console). 
