...
{{site.data.keyword.cloud_notm}} propose des portails de développeur s'appliquant à différents domaines d'intérêt (comme Watson) ou un canal de vente numérique (applications mobiles ou Web, par exemple). Vous pouvez accéder à ces portails à partir de l'icône **Menu** ![Icône Menu](../../icons/icon_hamburger.svg).
...
Si un kit de démarrage nécessite des services spécifiques, les instances de service mises à disposition automatiquement sont automatiquement créées et connectées à votre application.
