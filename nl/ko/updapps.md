...
### {{site.data.keyword.Bluemix_notm}} 사용자 인터페이스 사용:
  ...
  1. 조직의 사용자 정의 도메인을 작성하십시오.
	...
	1. **관리** &gt; **계정** &gt; **Cloud Foundry 조직** &gt; 조직의 **세부사항 보기**로 이동하십시오. 그런 다음 **Cloud Foundry 조직 편집** &gt; **도메인**을 클릭하십시오.
...
###예: bluemix app rename 명령 사용

이 예에서 애플리케이션의 이름은 Blue입니다. 이 예는 애플리케이션에 대한 트래픽을 중단하지 않고 **bluemix app rename** 명령을 사용하여 *Blue*의 버전을 업데이트하는 방법을 보여줍니다. 업데이트된 버전이 적절히 갖춰져 있으면 선택적으로 *Blue*의 이전 버전을 삭제할 수 있습니다.
...
2. **bluemix app rename** 명령을 사용하여 앱 이름을 *Blue*에서 *Green*으로 바꾸십시오.

  ```
  bluemix app rename Blue Green
  ```

  **bluemix app list** 명령을 사용하여 현재 영역의 애플리케이션을 나열하십시오.
...
4. 선택사항: 앱의 이전 버전(*Green*)을 삭제하려면 **bluemix app delete** 명령을 사용하십시오.
...
###예: bluemix app route-map 명령 사용

이 예에서 *Blue*는 이전에 배치한 애플리케이션이고, *Green*은 업데이트된 버전입니다. 이 예는 애플리케이션에 대한 트래픽을 중단하지 않고 **bluemix app route-map** 명령을 사용하여 *Blue*의 버전을 업데이트하는 방법을 보여줍니다. 업데이트된 버전이 적절히 갖춰져 있으면 선택적으로 *Blue*의 이전 버전을 삭제할 수 있습니다.
...
2. 필요한 사항을 변경하고 *Green* 버전을 준비 상태로 유지하십시오. *Green* 앱을 {{site.data.keyword.Bluemix_notm}}에 푸시하십시오.
  ...
  **bluemix app route-map** 명령을 사용하여 현재 영역의 애플리케이션을 나열하십시오.
...
3. `Blue.mybluemix.net`에 대한 모든 트래픽이 *Blue* 앱과 *Green* 앱에 라우트되도록 *Blue* 앱을 *Green* 앱에 맵핑하십시오.
  ...
  bluemix app route-map 명령을 사용하여 영역의 라우트 나열:
