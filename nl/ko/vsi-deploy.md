...
**중요**: 서비스는 가상 서버 인스턴스와 바인딩되지 않습니다. 가상 서버의 애플리케이션에는 서비스를 추가할 수 없습니다.
...
환경 특성 보기에 액세스하여 사용자 계정 특성으로 파이프라인을 사용으로 설정하려면 다음 단계를 따르십시오.

1. 앱 세부사항 페이지에서 **도구 체인 보기**를 클릭하십시오.
2. **Delivery Pipeline** 타일을 클릭하십시오.
3. **단계 구성** 아이콘을 클릭한 다음 빌드 단계에서 **단계 구성**을 클릭하십시오.
4. **환경 특성** 탭을 클릭하여 특성을 확인하십시오.
5. 도구 체인이 실행될 수 있도록 환경 특성 탭에서 이러한 특성을 사용으로 변경하십시오.
...
1. 인프라 [사용자 목록 ![외부 링크 아이콘](../icons/launch-glyph.svg)](https://control.bluemix.net/account/users){: new_window}으로 이동하십시오. **메뉴** > **인프라** > **계정** > **사용자 목록**을 클릭할 수도 있습니다.
...
Terraform에서 데이터베이스 및 작성(compose) 서비스와 같은 플랫폼 레벨 서비스를 작성하려면 플랫폼 API 키가 필요합니다. 다음 단계에 따라 플랫폼 키를 검색하십시오.
...
2. [인프라 SSH 키 보기 ![외부 링크 아이콘](../icons/launch-glyph.svg)](https://control.bluemix.net/devices/sshkeys){: new_window}로 이동하십시오. **메뉴** > **인프라** > **디바이스** > **관리** > **SSH 키**를 클릭할 수도 있습니다.
3. **추가**를 클릭하십시오.
...
Terraform은 Terraform `apply` 명령의 상태를 저장하도록 지원합니다. 이 상태 파일은 `apply` 명령을 두 번 실행하여 Terraform 구성 파일이 변경되었는지 여부를 판별합니다. 앱 및 구성이 관리되는 GIT 저장소에 상태를 저장할 수 있습니다.
